...
def getScore(school, gpa, hours, residency, cap):
    ...
    # Clean Data
    df20 = pd.read_csv('data/2020.csv', parse_dates=['Timestamp'])
    df21 = pd.read_csv('data/2021.csv', parse_dates=['Timestamp'])
    df21.rename(columns={
                "Credit Hours (Please include the exact number)": "Hours"}, inplace=True)
    # frames = [df20, df21]
    ...
    newdf20 = df20[['Decision', 'College', 'GPA',
                    'Hours', 'Residency', 'CAP']].copy()
    newdf21 = df21[['Decision', 'College', 'GPA',
                    'Hours', 'Residency', 'CAP']].copy()
    ...
    students = pd.DataFrame(
        columns=['College', 'GPA', 'Hours', 'Residency', 'CAP'])
